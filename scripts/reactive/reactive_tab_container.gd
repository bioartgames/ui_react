...
func _validate_animation_targets() -> void:
	var r = ReactiveAnimationTargetHelper.validate_and_map_triggers(
		self,
...
func _on_tab_selected(tab_index: int) -> void:
	if _previous_tab_index >= 0 and _previous_tab_index != tab_index:
		TabTransitionAnimator.animate_tab_switch(self, _previous_tab_index, tab_index, animation_targets)

	_previous_tab_index = tab_index

	TabContentStateBinder.bind_tab_content(self, tab_config, tab_index, Callable(self, "_on_tab_content_state_changed"))
...
func _on_selected_state_changed(new_value: Variant, _old_value: Variant) -> void:
	if _updating:
		return
	var index: int = TabSelectionBinding.resolve_tab_index(self, new_value)
	...
	if _previous_tab_index >= 0 and _previous_tab_index != index:
		TabTransitionAnimator.animate_tab_switch(self, _previous_tab_index, index, animation_targets)
	TabContentStateBinder.bind_tab_content(self, tab_config, index, Callable(self, "_on_tab_content_state_changed"))
...
func _on_tabs_state_changed(new_value: Variant, _old_value: Variant) -> void:
	...
	var prev_update = TabCollectionSync.apply_tabs_from_array(self, tabs_array, tab_config)
	if prev_update != null:
...
func _on_tab_content_state_changed(tab_index: int, property: String, new_value: Variant, _old_value: Variant) -> void:
	TabContentStateBinder.propagate_content_change(self, tab_index, property, new_value)
