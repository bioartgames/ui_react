...
**Next milestones (committal intent, not a release promise):** (1) **In-tab** editing of edge payloads where the details pane can replace ad hoc Inspector hops (beyond **rule_id**); (2) **Tab consolidation** — merge **Dependency Graph** and **Wire rules** into one **Wiring** workbench tab, **Diagnostics** unchanged; (3) optional: **resource creation** from the graph (factory flows mirroring Inspector). **Polish:** scope presets, pins, or filters so designers control the microcosm without noise.
