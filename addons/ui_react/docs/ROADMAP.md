...
| CB-063 | Wire-rule stack presets (authoring accelerators) | Inventory/detail, list-heavy screens | P6.2 | Planned | One-click insertion of vetted multi-row `wire_rules` recipes (e.g., sort + copy-detail + suffix/pulse), authored as normal subresources (no alternate on-disk format). Integrate with **CB-058** workbench and diagnostics parity. |
