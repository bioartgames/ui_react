...
The **Dependency Graph** tab (**CB-018A**–**CB-018A.5**) explains declarative **bindings**, **`wire_rules`**, and **`UiComputed*` `sources`**. **Authoring on the canvas (reconnect):** **Shift+drag** from a selected edge’s **source** endpoint to another **state/computed** node reuses the same undo-safe commits as the **Rebind…** buttons (**`CB-058`** step 2). **Still out of scope:** drawing **new** edges from scratch on the canvas, merging tabs, and **runtime** graph editing.

The **next major dock milestone** remains turning that surface into an **optional primary authoring tool** for **full** connect/disconnect—including **greenfield** links and richer snap targets—while still writing the **same** scene and resource shapes (**no** parallel format; **Charter** parity with **CB-035**).

**Sequencing (committal intent, not a release promise):** (1) **edit intents** (file picker) **done**; **canvas reconnect** for existing **binding / computed-source / wire-flow** edges **done**; (2) **canvas** **new-edge** creation and further snap-target polish; (3) **tab consolidation** only after parity—e.g. merge **Dependency Graph** and **Wire rules** into one **Wiring graph** tab, leaving **Diagnostics** as today. **Runtime** “edit the graph while playing” (**CB-018** remainder) stays **explicitly later**: persistence, authority, and dev-only vs shipped builds must be specified before it is promoted from narrative to Appendix **Done**.
