...
The **Dependency Graph** tab (**CB-018A**–**CB-018A.5**) today is **read-only**: it explains declarative **bindings**, **`wire_rules`**, and **`UiComputed*` `sources`** so designers see the whole picture without hopping Inspectors. The **next major dock milestone** is to turn that surface into an **optional primary authoring tool**: **drag-to-connect / reconnect** edges, add or remove wiring **without** treating the Inspector as the only place truth is edited—while still writing the **same** scene and resource shapes (**no** parallel format; **Charter** parity with **CB-035**).

**Sequencing (committal intent, not a release promise):** (1) **edit intents** and reconnect flows backed by **`EditorUndoRedoManager`** and existing validator/scanner paths; (2) **canvas** drag with **snap targets** and per-edge-kind rules; (3) **tab consolidation** only after parity—e.g. merge **Dependency Graph** and **Wire rules** into one **Wiring graph** tab, leaving **Diagnostics** as today. **Runtime** “edit the graph while playing” (**CB-018** remainder) stays **explicitly later**: persistence, authority, and dev-only vs shipped builds must be specified before it is promoted from narrative to Appendix **Done**.
