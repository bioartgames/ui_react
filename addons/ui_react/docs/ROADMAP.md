...
| CB-018 | State graph / debug overlay (editor or runtime) | Development | P6.3 | Planned | **CB-018C** (**3.2.0**): **`Output`** **`print`** orchestration slice (**`UiReactRuntimeConsoleDebug`**, Wiring-checkbox persistence). Promoted **2026-04-17**. **Editor** graph MVP = **CB-018A**. **Defer:** Dependency Graph highlights during play, ring buffer / exhaustive state deltas before any runtime authoring. |
| CB-018C | Runtime **Output** orchestration trace (wire / computed / actions) | Development | P6.3 | Done | **`scripts/runtime/ui_react_runtime_console_debug.gd`** + taps on **`UiReactWireRuleHelper`**, **`UiReactComputedService`**, **`UiReactActionTargetHelper`**; toggle **`Wiring`** tab **`CheckBox`** only (**`console_debug_enabled`**). |
