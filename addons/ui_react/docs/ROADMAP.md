...
| CB-018 | State graph / debug overlay (editor or runtime) | Development | P6.3 | Planned | **Editor MVP:** **CB-018A**–**CB-018A.5**. **Runtime observer MVP** shipped as **CB-018C** (**`3.2.0`**) — read-only **`Alt+3`** overlay + ring buffer; full **CB-018** backlog (timeline/causality depth, per-selection UX) stays **planned** without runtime authoring. |
...
| CB-018C | Runtime live debug overlay (**read-only trace** / ring buffer / **`Alt+3`**) | Development | P6.3 | Done | **`UiReactLiveDebug`** autoload, **`ui_react_live_debug_bridge.gd`** (+ façade **`ui_react_live_debug_facade.gd`**), **`UiReactLiveDebugBuffer`**, **`UiReactLiveDebugHarvester`**, overlay script; taps in wire/computed/action helpers; **`UiReactGraphNodeIds`** aligns stable ids with **Dependency Graph**. **Manual autoload registration** — see README (**`3.2.0`**). Requires **debug build** + PS **`live_debug_enabled`**. |
