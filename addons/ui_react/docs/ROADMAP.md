...
**Done:** **CB-049**, **CB-050** (**2.17.0**); **CB-052**, **CB-054**, **CB-055** (**2.18.0** surface); **CB-056** (**2.19.0** / **2.19.1**); **CB-057** (**2.20.0**); **CB-051** (**2.21.0**); **CB-035** (**2.22.0** — dock **Wire rules** tab). **Remaining:** **CB-053** (broader plugin UX after parity — e.g. bulk anim helpers; non-blocking). **North star (designer tooling):** **CB-058** — **Dependency Graph** as the **blessed editor workbench** for wiring (see below).
...
The **Dependency Graph** tab (**CB-018A**–**CB-018A.5**) is the **primary** surface for this workbench; **Wire rules** tab and **Inspector** are **peers** on the **same** resource model (**DRY**).

**Shipped authoring:** **Rebind…** buttons and file pickers; **disconnect** — **Clear optional binding** / **Remove computed dependency** (optional exports and computed **`sources[]`** slots only; **Delete**/**Backspace** when an edge is selected); **canvas reconnect** — **Shift+drag** from a selected edge’s **source** endpoint to another **state/computed** node; **canvas new link** — **Ctrl+Shift+drag** (no edge selected) from a **state/computed** donor to a **control** (null registry binding) or **embedded** **computed** (**`sources[]`** fill/append); **undo** coherency includes graph refresh on **`EditorUndoRedoManager.version_changed`**.
...
**Next milestones (committal intent, not a release promise):** (1) **In-tab** editing of edge payloads where the details pane can replace ad hoc Inspector hops; (2) **Wire-flow greenfield** and **file-backed** computed targets on canvas **via the same commit paths** as **Wire rules** / resources (with clear UX for shared **`.tres`**); (3) **Tab consolidation** — merge **Dependency Graph** and **Wire rules** into one **Wiring** workbench tab, **Diagnostics** unchanged; (4) optional: **resource creation** from the graph (factory flows mirroring Inspector). **Polish:** scope presets, pins, or filters so designers control the microcosm without noise.
...
| **Inspector-first story (docs + examples)** | **Official examples** prioritize **Inspector** resources (**`wire_rules`**, **`UiComputed*`**, **transactional**, **`action_targets`**) over **scene scripts** for the layers the addon models—so **proof** and **diffs** stay objective (**evidence bar**). **Designer path in the editor:** **Dependency Graph** (**CB-058**) is the **blessed** place to orchestrate those **same** resources (see **North star** above); Inspector remains **fully capable** for authors who skip the graph. README states the **four pillars** + **designer path**. **Play** validates runtime; **domain** logic and **non-goals** items remain game code. |
...
| CB-058 | **Dependency Graph** as designer **wiring workbench** (visual orchestration; **no** parallel format) | Editor | P6+ | Planned | **CB-018A** snapshot + canvas authoring (reconnect, new link, rebinds, undo refresh). **Target:** disconnect / edge editing in-tab, wire greenfield + file-backed targets, resource factories, **Wiring** tab merge (**Dependency Graph** + **Wire rules**). **Runtime** graph: **CB-018**. Normative narrative: Part I **North star** + **Visual wiring graph**. |

---

*Last updated: 2026-04-09 — **North star:** **Dependency Graph** documented as **blessed designer workbench** (Inspector remains full parity). **CB-058** Appendix row + roadmap narrative; shipped: reconnect, **2b** new link, rebinds, **version_changed** graph refresh. **Next:** disconnect, in-tab edge editing, wire/file-backed canvas, **Wiring** tab merge. **CB-018A.5**–**CB-018A** graph lineage unchanged. **CB-053** deferred. Quarterly review: **Review process** → **Last quarterly review**. Stock-take: [`P5_CURRENT_STATE_AUDIT.md`](P5_CURRENT_STATE_AUDIT.md). Examples remain **four** scenes.*
