...
| CB-018A.4 | Dependency Graph details (editor): high-value copy + `node_layer` | Development | P6+ | Done | **`node_layer`** on layout snapshot; explain panel **headline**, **relative-to-focus**, **incident edges**, **cycles**, **Text** pointer, **technical** last; edge **lead** + **where to edit** + **focus** note. |
...
*Last updated: 2026-04-09 — **CB-018A.4**: **Dependency Graph** details pane copy + **`node_layer`**. **CB-018A.3**: **Dependency Graph** tab UX (auto-refresh, legend, inspector focus). **CB-018A.2**: Visual readability. **CB-018A.1**: Visual graph MVP. **CB-018A**: declarative graph. Prior: **2.22.0**: **CB-035** / **P5.2** dock **Wire rules** tab. **CB-053** remains (broader plugin UX). Quarterly review: **Review process** → **Last quarterly review**. Stock-take: [`P5_CURRENT_STATE_AUDIT.md`](P5_CURRENT_STATE_AUDIT.md). Examples remain **four** scenes.*
