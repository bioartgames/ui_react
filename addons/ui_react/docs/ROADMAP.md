...
**Done:** **CB-049**, **CB-050** (**2.17.0**); **CB-052**, **CB-054**, **CB-055** (**2.18.0** surface); **CB-056** (**2.19.0** / **2.19.1**); **CB-057** (**2.20.0**); **CB-051** (**2.21.0**); **CB-035** (**2.22.0** — dock **Wire rules** tab). **Remaining:** **CB-053** (broader plugin UX after parity — e.g. graph-style wire view, bulk anim helpers; non-blocking). **Planned north star:** **CB-058** (visual wiring graph — see below).

### Visual wiring graph (planned — CB-058)

The **Dependency Graph** tab (**CB-018A**–**CB-018A.5**) explains declarative **bindings**, **`wire_rules`**, and **`UiComputed*` `sources`**. **Authoring on the canvas (reconnect):** **Shift+drag** from a selected edge’s **source** endpoint to another **state/computed** node reuses the same undo-safe commits as the **Rebind…** buttons (**`CB-058`** step 2). **Authoring (greenfield, phase 2b):** **Ctrl+Shift+drag** from a **state/computed** donor to a **control** (null binding slot) or **embedded** **computed** ( **`sources[]`** fill/append ) — **not** file-backed computed targets on canvas, **not** new **wire-flow** rows (use **Wire rules** tab). Merging tabs and **runtime** graph editing stay out of scope.

The **next major dock milestone** remains turning that surface into an **optional primary authoring tool** for **full** connect/disconnect—including richer snap targets and any remaining greenfield cases—while still writing the **same** scene and resource shapes (**no** parallel format; **Charter** parity with **CB-035**).

**Sequencing (committal intent, not a release promise):** (1) **edit intents** (file picker) **done**; **canvas reconnect** for existing **binding / computed-source / wire-flow** edges **done**; (2) **canvas** **new-edge** creation (**binding + embedded computed `sources`**, phase **2b**) **done**; further polish / optional targets **TBD**; (3) **tab consolidation** only after parity—e.g. merge **Dependency Graph** and **Wire rules** into one **Wiring graph** tab, leaving **Diagnostics** as today. **Runtime** “edit the graph while playing” (**CB-018** remainder) stays **explicitly later**: persistence, authority, and dev-only vs shipped builds must be specified before it is promoted from narrative to Appendix **Done**.
...
*Last updated: 2026-04-09 — **CB-058**: **Visual wiring graph** — **canvas reconnect** (**Shift+drag**) and **phase 2b** **new link** (**Ctrl+Shift+drag**, binding + embedded computed **`sources`**) shipped; **new wire-flow** on canvas + **file-backed** computed targets + **Wire rules** tab merge + **runtime** (**CB-018**) still tracked below. **CB-018A.5**: **Dependency Graph** graph-only + per-anchor narrative. **CB-018A.4**: details pane + **`node_layer`**. **CB-018A.3**: tab UX. **CB-018A.2**: Visual readability. **CB-018A.1**: Visual graph MVP. **CB-018A**: declarative graph. Prior: **2.22.0**: **CB-035** / **P5.2** dock **Wire rules** tab. **CB-053** remains (broader plugin UX; **CB-058** subsumes graph-style slice). Quarterly review: **Review process** → **Last quarterly review**. Stock-take: [`P5_CURRENT_STATE_AUDIT.md`](P5_CURRENT_STATE_AUDIT.md). Examples remain **four** scenes.*
