...
A compact **health summary** in the dock reflects the **current editor selection** (and its bound `UiState` properties): pass/warn/fail counts and quick navigation to related issues.
...
- Summarize binding health using existing scanner/validator output (filter issues to selected node/property where possible).
- Click/select affordance to focus first matching issue in the list (if list supports it).

**Out of scope (YAGNI)**

- Graph of historical health over time.
- Per-frame runtime telemetry (runtime bridge plan).

## 3) Files to change

- `addons/ui_system/editor_plugin/ui_system_dock.gd` — card UI + subscribe to `EditorSelection` (or equivalent Godot 4 editor API).
- `addons/ui_system/editor_plugin/services/ui_system_scanner_service.gd` — optional: index issues by node path for O(1) lookup.
- `addons/ui_system/editor_plugin/services/ui_system_validator_service.gd` — ensure issue payloads carry **node path** + **property path** consistently.
...
- Mismatch: if selection has no `UiReact*` bindings, short hint **No UiReact bindings on this node** (link to wizard plan optional).
