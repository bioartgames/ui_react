...
- **Editor dock — Selected wire rule details:** the **Validation warnings** row in [`UiReactDockWireDetails`](../editor_plugin/dock/ui_react_dock_wire_details.gd) is built from **filtered** [`UiReactWiringValidator.validate_wire_rules`](../editor_plugin/services/ui_react_wiring_validator.gd) issues (same `issue_text` / `fix_hint` as **Diagnostics**), replacing the previous duplicate heuristic. **`UiReactWiringValidator`** now reports disabled rules, empty trimmed **`sort_key_state`** for **`UiReactWireSortArrayByKey`**, and unknown concrete rule classes. Cross-host **duplicate rule instance** messages (`wire_rules: same UiReactWireRule instance…`) remain Diagnostics-only (no `wire_rules[N]:` prefix to filter on).
