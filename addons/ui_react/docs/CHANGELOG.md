...
- **Editor plugin — plugin-only settings surface:** removed user-facing Ui React Project Settings tab exposure and moved shortcut UX to an in-dock **Settings** popup.
- **Editor plugin — dual action shortcuts:** two assignable shortcuts (`open_diagnostics_json`, `open_wiring_json`) with capture UI; defaults **Alt+1** / **Alt+2**; both open/select the Ui React bottom panel when needed and apply without editor restart.
