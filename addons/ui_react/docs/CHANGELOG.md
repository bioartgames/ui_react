...
- **Editor plugin — bottom dock tab:** the **Ui React** bottom-panel tab now disables **`BaseButton.shortcut_in_tooltip`** and renders a single tooltip line directly from the configured binding (default **`Toggle Ui React Bottom Panel (Alt+U)`**), eliminating the editor-added shortcut header/newline. Default shortcut remains **Alt+U** (Godot bottom-panel convention). Customize or disable via **Project Settings** key **`ui_react/plugin_editor_bottom_panel_shortcut_json`** (JSON schema v1: `v`, optional `enabled`, `keycode`, `alt` / `shift` / `ctrl` / `meta`). Use **`{}`** or **`"enabled":false`** to clear the shortcut. Invalid JSON falls back to **Alt+U**; changing the value **re-registers** the tab without restarting the editor. Implementation: **`UiReactEditorBottomPanelShortcut`**, **`UiReactDockConfig.KEY_EDITOR_BOTTOM_PANEL_SHORTCUT_JSON`**, **`ui_react_editor_plugin.gd`**.
