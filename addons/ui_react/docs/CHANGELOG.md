...
- **Editor dock — Dependency Graph (**`CB-058`**, slice 1)**: **Rebind to resource…** on a selected **binding** edge — undo-safe assignment via **`UiReactActionController`**, file picker for a **`UiState`** `.tres`, and co-refresh of dock **Diagnostics**. Wire-flow / computed-source reconnect deferred.
