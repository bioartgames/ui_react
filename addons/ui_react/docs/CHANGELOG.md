...
- **Editor dock — Wiring / Dependency Graph — chrome**: selection-scoped commands (**rebind**, **clear/remove**, **computed source** reorder, **Create & assign…**, **Copy details**) are combined in an **Actions…** `MenuButton` and the same entries appear on **right-click** on the graph canvas; **Save as…** / **Manage…** / **Pin node** for scope presets are under a **…** overflow next to the preset dropdown (**`UiReactDockExplainPanel`**, **`UiReactExplainGraphView.context_menu_requested`**).
