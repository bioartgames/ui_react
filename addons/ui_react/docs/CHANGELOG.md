...
- **Project Settings — runtime live debug:** **`live_debug_enabled`** and **`live_debug_buffer_cap`** use **`ProjectSettings.add_property_info`** and **`set_as_internal(..., false)`** so they show under **`ui_react`** (search **`live debug`**); other **`ui_react/settings/...`** keys remain internal. README **Runtime live debug** updated.
...
- **Runtime live debug overlay (**`CB-018C`**)**: **`UiReactLiveDebug`** autoload (`scripts/runtime/ui_react_live_debug.gd`) gates on **`OS.is_debug_build()`** and Project Settings **`ui_react/settings/runtime/live_debug_enabled`** (defaults **false**); ring buffer capacity **`live_debug_buffer_cap`** (clamp **64**–**2048**, default **384** — see **`UiReactDockConfig.live_debug_buffer_cap_effective`**). **`ui_react_live_debug_bridge.gd`** → **`ui_react_live_debug_facade.gd`** **`maybe_*`** records **STATE_VALUE_CHANGED** (harvester **`UiReactLiveDebugHarvester`** + **`UiReactComponentRegistry`** / **`UiReactScannerService`** introspection paths), **COMPUTED_RECOMPUTE**, **WIRE_RULE_APPLY**, **ACTION_APPLY**. **`Alt+3`** toggles a **`CanvasLayer`** overlay (`ui_react_live_debug_overlay.gd`). Stable ids use **`UiReactGraphNodeIds`** (editor **`UiReactExplainGraphBuilder`** refactored); GUT **`test_ui_react_graph_node_ids.gd`**, **`test_ui_react_live_debug_buffer.gd`**. **`editor_plugin/plugin.cfg`** **`version`** **3.2.0**.
