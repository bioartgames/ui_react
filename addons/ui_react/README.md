...
2. Open the **Ui React** panel in the **bottom editor dock** (tab bar: **Diagnostics**, **Wire rules**, **Explain**).
3. Choose **Scan: Selection** or **Entire scene**, press **Rescan** to run diagnostics on demand, and review results. Dock choices (scan mode, **Group** mode, filters, auto-refresh, output folder) are **remembered per project** when you reopen it. The tool also **updates when you switch the active edited scene**, and when **EditorFileSystem** reports filesystem changes (coalesced refresh so rapid imports do not spam rescans). Open the **Explain** tab to **Refresh explain** after selecting a single **`UiReact*`** node — **Text** mode shows the full BBCode report (upstream/downstream, cycle candidates); **Visual** mode (**`CB-018A.1`** + **`CB-018A.2`**) shows a read-only scoped graph (**orthogonal** edges, **short** on-canvas names, **filters**, bottom **details** + breadcrumb, pan/zoom, framed viewport; declarative only, not a runtime trace).
