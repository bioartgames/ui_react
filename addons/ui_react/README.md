...
3. Choose **Scan: Selection** or **Entire scene**, press **Rescan** to run diagnostics on demand, and review results. Dock choices (scan mode, **Group** mode, filters, auto-refresh, output folder) are **remembered per project** when you reopen it. The tool also **updates when you switch the active edited scene**, and when **EditorFileSystem** reports filesystem changes (coalesced refresh so rapid imports do not spam rescans). Open the **Dependency Graph** tab: the graph **refreshes when your scene selection changes** (debounced); use **Refresh** to rebuild after edits. Select a single **`UiReact*`** node — **Text** mode shows the full BBCode report (upstream/downstream, cycle candidates); **Visual** mode (**`CB-018A.1`**–**`CB-018A.3`**) shows a read-only scoped graph (**orthogonal** edges, **short** on-canvas names, **legend**, **filters**, bottom **details**, **Focus in Inspector** / **Copy details**, pan/zoom, framed viewport; declarative only, not a runtime trace).
