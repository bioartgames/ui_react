...
**Persisted per project:** scan mode, **Group** mode, severity filters, auto-refresh, state output folder, ignored unused file paths (**`ui_react/plugin_ignored_unused_state_paths`**), **last dock tab** (**`ui_react/plugin_dock_last_tab`**), and **Wiring return state** (**`ui_react/plugin_wiring_last_scene_path`**, **`ui_react/plugin_wiring_last_scope_node_path`**, **`ui_react/plugin_wiring_last_graph_node_id`**) are saved in **Project Settings** and restored when you reopen the project (no need to reconfigure each session).
