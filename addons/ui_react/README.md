...
| **Transactional** | Draft vs committed **Apply / Cancel** | **`UiTransactionalState`**, **`UiTransactionalGroup`**, **`UiTransactionalScreenConfig`**, **`UiReactButton`** / **`UiReactTextureButton`** **`transactional_*`** + **`UiReactTransactionalSession`** (see **Transactional state**); **`UiReactTransactionalActions`** is **deprecated** (path-based only). |
...
- **Blessed path (typical full screen):** attach **`UiReact*`** → assign **`*_state`** → optional **`animation_targets`** → optional **`wire_rules`** ( **`UiReactWireRuleHelper`** registers from each host; author rows in the Inspector **or** the **Ui React** dock **Wire rules** tab—same resources, **CB-035** / **P5.2** ) → optional **`UiComputed*`** on bindings → optional **`UiTransactionalGroup`** + Apply/Cancel on **`UiReactButton`** / **`UiReactTextureButton`** (**`transactional_group`**, **`UiTransactionalScreenConfig`**, **`transactional_role`**) → optional **`action_targets`**. Official **examples** (below) mix these layers.
...
|--------|--------------------|----------------------------------|
| **UiReactButton** | `pressed_state`, `disabled_state`, optional **`transactional_group`** / **`transactional_screen`** / **`transactional_role`** | At least one state if you want sync with `UiState`; neither required for a plain Button. **`disabled_state`** accepts **`UiBoolState`** or a **`UiComputedBoolState`** subclass. For Apply/Cancel, set **`transactional_role`** and omit **`pressed_state`** (validator warns if both). |
...
| **UiReactTextureButton** | `pressed_state`, `disabled_state`, optional **`transactional_*`**, optional `action_targets` | Same as **`UiReactButton`** (including transactional Apply/Cancel). Assign **`texture_normal`** (and optional hover/pressed textures) in the Inspector. |
...
**Runnable example:** **`res://addons/ui_react/examples/options_transactional_demo.tscn`** — master volume (**`HSlider`** + **`UiReactSlider`**) and mute (**`CheckBox`** + **`UiReactCheckBox`**) share transactional resources; **Apply** / **Cancel** use **`UiReactButton`** with **`transactional_group`**, shared **`UiTransactionalScreenConfig`**, and **`transactional_role`** (**`UiReactTransactionalSession`**). The demo status line uses **`UiReactLabel`** + **`UiComputedTransactionalStatusString`** on **`text_state`** (stock addon computed; or another **`UiComputedStringState`** subclass) so draft / committed / pending text stays in sync (see **Computed state**).
...
4. On the **Apply** and **Cancel** **`Button`** / **`TextureButton`** nodes, attach **`UiReactButton`** / **`UiReactTextureButton`**. Assign the **same** **`UiTransactionalGroup`** and **same** **`UiTransactionalScreenConfig`** on both; set **`transactional_role`** to **Apply** vs **Cancel** (`UiReactTransactionalSession.Role`).
...
**Deprecated — `UiReactTransactionalActions`:** path-based Apply/Cancel only; do not combine with button **`transactional_*`** on the same group (dock **ERROR**).
