...
**Maintainer / agent entrypoint:** **[`AGENTS.md`](AGENTS.md)** (documentation map, scope, paths, boundaries, change policy for this addon).

### Four pillars (resources-first; examples stay Inspector-authored)

**Designer tooling:** In the **editor**, the **Wiring** tab (**Dependency Graph** + **`wire_rules`** list, Ui React dock) is the **blessed** place to **see and orchestrate** wiring, computeds, and dataflow in a scoped graph—**same** resources as the Inspector (**[`docs/ROADMAP.md`](docs/ROADMAP.md)** **North star**). Subclasses of **`UiComputedStringState`** / **`UiComputedBoolState`** (the shipped **`UiComputed*`** family) expose **`sources[]`** in the graph with the same **Shift+drag reconnect** and **Rebind computed source…** flow as bindings (**CB-058**). **Official examples** remain edited as **Inspector** resources in **`.tscn`** so the **Charter** evidence bar stays objective.
...
- **Blessed path (typical full screen):** attach **`UiReact*`** → assign **`*_state`** → optional **`animation_targets`** → optional **`wire_rules`** on **[`WIRING_LAYER.md`](docs/WIRING_LAYER.md)** §5 hosts (**`UiReactWireRuleHelper`** registers from each **`wire_rules`** export; author in the dock **Wiring** tab **or** Inspector—**same** resources, **CB-035** / **P5.2** / **CB-058** ) → optional **`UiComputed*`** on bindings → optional **`UiTransactionalGroup`** + Apply/Cancel on **`UiReactButton`** / **`UiReactTextureButton`** (**`transactional_host`**, shared **`UiTransactionalScreenConfig`** on the binding) → optional **`action_targets`**. Official **examples** (below) mix these layers and stay **Inspector-authored** for **objective** proof.
...
2. Open the **Ui React** panel in the **bottom editor dock** (tab bar: **Diagnostics**, **Wiring**). The tab has **no** global toggle shortcut; hover the **Ui React** bottom-panel tab for **`Toggle Ui React Bottom Panel (Alt+1, Alt+2)`** by default (main row digits, not numpad). Tooltip text follows the internal **`open_diagnostics_json`** / **`open_wiring_json`** values (see **Project settings** below).
3. Choose **Scan: Selection** or **Entire scene**, press **Rescan** to run diagnostics on demand, and review results. **Runtime console trace (**`CB-018C`** v1):** On **Wiring**, at the **bottom** of the tab, enable **Print runtime trace to Output (debug builds only)** so **`UiReactRuntimeConsoleDebug`** prints **`WIRE`**, **`CMP`**, **`ACT`** lines to **Output** in **debug** builds (wire rule **`apply`**, computed **`recompute`**, **`action_targets`** dispatch—not exhaustive **`UiState` mutation`). Persists **`ui_react/settings/runtime/console_debug_enabled`** (written **only** from **`UiReactDockConfig.save_ui_preference`** via the Wiring tab checkbox). Applies on **next** play run. Dock choices (scan mode, **Group** mode, filters, auto-refresh, output folder) are **remembered per project** when you reopen it. The **last dock tab** (**Diagnostics** vs **Wiring**) is remembered too; if **Wiring** was last, the editor restores the **Wiring** scope control and (when valid) the last **graph node** selection for that edited scene. The **Diagnostics** tab title shows the **filtered visible issue count** (e.g. `Diagnostics (12)`). The tool also **updates when you switch the active edited scene**, and when **EditorFileSystem** reports filesystem changes (coalesced refresh so rapid imports do not spam rescans). Open the **Wiring** tab for the **recommended** workbench (**[`docs/ROADMAP.md`](docs/ROADMAP.md)** **North star**): the **Dependency Graph** (left) **refreshes when your scene selection changes** (debounced), after **undo/redo** (**`EditorUndoRedoManager.version_changed`**), and when you press **Refresh**; the **`wire_rules`** list (right) stays in sync. Select a single **`UiReact*`** node to set scope; the **details** pane holds the narrative report (upstream/downstream, cycle candidates) and **Visual** graph (**`CB-018A.1`**–**`CB-018A.5`**) shows a **scoped** canvas (**orthogonal (Manhattan)** edges, **legend**, **filters**, **Focus in Inspector** / **Copy details**, pan/zoom, canvas **reconnect** / **new link** where implemented—declarative snapshot, not a runtime trace).
...
|--------|--------------------|----------------------------------|
| **UiReactButton** | `pressed_state`, `disabled_state`, optional **`transactional_host`**, optional `animation_targets`, optional `action_targets` (no `wire_rules`; wiring pickers live on **[`WIRING_LAYER.md`](docs/WIRING_LAYER.md)** §5 hosts) | At least one state if you want sync with `UiState`; neither required for a plain Button. **`disabled_state`** accepts **`UiBoolState`** or a **`UiComputedBoolState`** subclass. For Apply/Cancel, set **`transactional_host.role`** and omit **`pressed_state`** (validator warns if both). |
| **UiReactCheckBox** | `checked_state`, `disabled_state`, optional `animation_targets`, optional `action_targets`, optional `wire_rules` | Assign `checked_state` (**`UiState`**: **`UiBoolState`** or bool-shaped **`UiTransactionalState`**) for two-way sync. |
| **UiReactSlider** | `value_state`, optional `animation_targets`; **no** `action_targets` / **`wire_rules`** yet (**[`ROADMAP.md`](docs/ROADMAP.md)** **CB-052** **†** / **CB-060**) | Assign **`value_state`** (**`UiState`**: **`UiFloatState`** or float/int-shaped **`UiTransactionalState`**) for two-way sync; else behaves like a normal slider. |
| **UiReactSpinBox** | `value_state`, `disabled_state`, optional `animation_targets`; **no** `action_targets` / **`wire_rules`** yet (**CB-052** **†** / **CB-060**) | Same **`value_state`** pattern as slider; `disabled_state` optional (**`UiBoolState`** or **`UiComputedBoolState`** subclass). |
| **UiReactProgressBar** | `value_state`, optional `animation_targets`; **no** `action_targets` / **`wire_rules`** yet (**CB-052** **†** / **CB-060**) | Same **`value_state`** pattern as slider. |
| **UiReactLineEdit** | `text_state`, optional `animation_targets`, optional `action_targets`, optional `wire_rules` | `text_state` for sync. |
| **UiReactLabel** | `text_state`, optional `animation_targets` (labels do **not** host **`action_targets`** / **`wire_rules`** — **CB-052** **—**) | `text_state` for sync. |
| **UiReactRichTextLabel** | `text_state`, optional `animation_targets` (same **—** as **`UiReactLabel`** for **`action_targets`** / **`wire_rules`**) | **`text_state`** for **display-only** sync (BBCode string → **`RichTextLabel.text`**); wrapper forces **`bbcode_enabled`**. Same payload family as **`UiReactLabel`**; **no** UI→state writeback. |
...
| **UiReactItemList** | `items_state`, `selected_state`, optional `animation_targets`, optional `action_targets`, optional `wire_rules` | **`items_state`**: **`UiArrayState`** (`Array` of strings/variants or **`label`/`icon`** dicts per **List patterns**). **`selected_state`**: **`UiIntState`** (single-select) or **`UiArrayState`** (multi-select indices). Godot’s **ItemList** has no built-in disabled state—wrap or gate input with a parent **Control** / `mouse_filter` / focus policy in game code if you need “disabled” behavior. |
...
| **UiReactTextureButton** | `pressed_state`, `disabled_state`, optional **`transactional_host`**, optional `animation_targets`, optional `action_targets` (no `wire_rules`; same **`UiReactButton`** split) | Same as **`UiReactButton`** (including transactional Apply/Cancel). Assign **`texture_normal`** (and optional hover/pressed textures) in the Inspector. |
| **UiReactTree** | `tree_items_state`, `selected_state`, optional `animation_targets`, optional `action_targets`, optional `wire_rules` | **`tree_items_state`**: **`UiArrayState`** — value is an **`Array` of `UiReactTreeNode`** (see below). **`selected_state`**: **`UiIntState`**. Wrapper forces **single selection** (`Tree.SELECT_SINGLE`). See **UiReactTree binding semantics** below. |
...
| Runtime Output trace (**debug**, Wiring-tab opt-in **`CB-018C`** v1) | `UiReactRuntimeConsoleDebug` | `scripts/runtime/ui_react_runtime_console_debug.gd` |
...
| `docs/` | **CHANGELOG**, **[`DECISIONS.md`](docs/DECISIONS.md)**, **[`ROADMAP.md`](docs/ROADMAP.md)**, **[`TESTING.md`](docs/TESTING.md)** (GUT rollout), **[`WIRING_LAYER.md`](docs/WIRING_LAYER.md)** (normative **P5** wiring), **[`ACTION_LAYER.md`](docs/ACTION_LAYER.md)** (normative **P6.1** actions), **[`FEEDBACK_LAYER.md`](docs/FEEDBACK_LAYER.md)** (normative **P6.3** feedback). **[`AGENTS.md`](AGENTS.md)** (addon root — maintainer map + checklist). |
