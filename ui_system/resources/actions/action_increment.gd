## Action that increments a numeric ReactiveValue.
@icon("res://icon.svg")
class_name ActionIncrement
...
func validate_before_execute(target: ReactiveValue, params: ActionParams) -> bool:
	...
	if not (params is IncrementParams):
		return false
...
## Increments the target value by the amount specified in params.
